...
- **Typed `generateOpenAPI` signature and `OpenAPIDocument` return type.** (#124) The
  `generateOpenAPI` function's `router` parameter and return type were both `object` in the
  generated `.d.ts`, forcing consumers to cast. The `router` parameter is now typed as `Router`
  and the return type is `OpenAPIDocument`, a new interface modeling the OpenAPI 3.1 document
