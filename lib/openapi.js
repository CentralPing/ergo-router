...
const DEFAULT_RESPONSES = {
  POST: {status: 201, description: 'Resource created'},
  DELETE: {status: 204, description: 'No content'}
};
...
function buildOperation(entry, fallbackDefaults, securitySchemes) {
  ...
  if (!operation.responses) {
    const methodDefault = DEFAULT_RESPONSES[method];
    operation.responses = methodDefault
      ? {[methodDefault.status]: {description: methodDefault.description}}
