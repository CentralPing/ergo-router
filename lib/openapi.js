...
/**
 * Resolve a config key against defaults. Mirrors the resolution strategy
 * in `pipeline-builder.js`: route config > router defaults > omitted.
 *
 * @param {*} routeValue - Per-route config value
 * @param {*} defaultValue - Router-level default value
 * @returns {object|false|undefined} - Resolved options, `false` if disabled, `undefined` if absent
 */
 ...
 * Named params: `:id` → `{id}`. Wildcards: `*` → `{wildcard}`.
 *
...
function buildRequestBody(bodySchema, contentTypes) {
  const mediaType = contentTypes?.[0] ?? 'application/json';
  return {
    required: true,
    content: {
      [mediaType]: {schema: bodySchema}
    }
  };
}
 ...
 *
 * @param {object} entry - Route metadata entry `{method, path, config}`
 * @param {object} defaults - Router-level defaults from `router._options.defaults`
 * @param {object} securitySchemes - Mutable map collecting security schemes for `components`
 * @returns {object} - OpenAPI operation object
 */
function buildOperation(entry, defaults, securitySchemes) {
  ...
  if (annotations.summary) operation.summary = annotations.summary;
  if (annotations.description) operation.description = annotations.description;
  if (annotations.operationId) operation.operationId = annotations.operationId;
  if (annotations.tags) operation.tags = annotations.tags;
  if (annotations.deprecated !== undefined) operation.deprecated = annotations.deprecated;
  if (annotations.externalDocs) operation.externalDocs = annotations.externalDocs;
  ...
  if (annotations.responses) {
    operation.responses = annotations.responses;
  } else {
 ...
 * each extractable key against `router._options.defaults` using the same
 * precedence as the pipeline builder (route > defaults > omitted).
 *
 * @param {object} router - An ergo-router instance created by `createRouter()`
 * @param {object} [options] - OpenAPI document options
 ...
 * @returns {object} - OpenAPI 3.1 specification document
 */
export default function generateOpenAPI(router, options = {}) {
  ...
  for (const entry of router._routes) {
    ...
    paths[oaPath][methodKey] = buildOperation(entry, defaults, securitySchemes);
  }
