 ...
 * @param {function} [options.formatError] - Custom error formatter `(err, req, res) => sendableResult`
...
/**
 * Mapping from declarative config keys to accumulator keys for header-producing middleware.
 * Used to auto-compute the `headerKeys` option passed to `send()`.
 */
const HEADER_KEY_MAP = {securityHeaders: 'security', cacheControl: 'cache', rateLimit: 'rateLimit'};

/**
 * Compute which accumulator keys hold header tuples based on active middleware.
 * @param {object} routeConfig - Declarative route config
 * @param {object} defaults - Router-level defaults
 * @returns {string[]} - Array of accumulator key names
 */
function computeHeaderKeys(routeConfig, defaults) {
  const keys = [];
  for (const [configKey, accKey] of Object.entries(HEADER_KEY_MAP)) {
    const value =
      routeConfig[configKey] !== undefined ? routeConfig[configKey] : defaults[configKey];
    if (value !== false && value !== undefined) keys.push(accKey);
  }
  return keys;
}
...
function addRoute(router, method, path, pipeline, routeOpts = {}) {
  ...
  if (isConfigObject(pipeline)) {
    ...
    const headerKeys = computeHeaderKeys(pipeline, _options.defaults || {});
    const sendOpts = {};
    if (headerKeys.length) sendOpts.headerKeys = headerKeys;
    const preferValue =
      pipeline.prefer !== undefined ? pipeline.prefer : (_options.defaults ?? {}).prefer;
    if (preferValue !== false && preferValue !== undefined) sendOpts.preferKey = 'prefer';
    if (Object.keys(sendOpts).length) {
      effectiveOpts.send = {...(effectiveOpts.send ?? {}), ...sendOpts};
    }
  }
