 ...
 * @param {function} [options.onResponse] - Router-level observation hook.
 *   Fires on every response: both pipeline-routed responses (after send()) and
 *   transport-level short-circuits (404, 405, 415, 429, OPTIONS, CORS preflight).
 *   For pipeline responses, route-level hooks fire first, then the router-level hook.
 *   Receives `(req, res, responseInfo, domainAcc)` where `domainAcc` is `undefined`
 *   for transport-level responses (no pipeline ran). `responseInfo.source` is
 *   `'pipeline'` or `'transport'`. Hook errors are swallowed.
 * @param {Set<string>} [options.redactHeaders] - Header names to replace with `'[REDACTED]'`
 *   in `responseInfo.headers` passed to `onResponse` hooks. Defaults to
 *   `DEFAULT_REDACTED_HEADERS` (authorization, proxy-authorization, cookie, set-cookie).
 *   Pass an empty Set to disable redaction. Route-level `redactHeaders` overrides this.
 * @param {boolean} [options.strict=true] - If true, throw on unknown config keys; if false, warn
 ...
 * @param {boolean|object} [options.timing=false] - Inject an `X-Response-Time` header measuring
...
export default function createRouter(options = {}) {
  ...
  validateRouterOptions(options, strict);

  if (options.defaults) {
    validateDefaults(options.defaults, strict);
  }
  ...
  const redactSet = options.redactHeaders ?? new Set(DEFAULT_REDACTED_HEADERS);

  /** @type {Array<{method: string, path: string, config: object|undefined}>} */
  const routes = [];
  ...
  const router = {
     ...
     * @param {function | function[] | import('./pipeline-builder.js').RouteConfig} pipeline - Handler function, pipeline array, or declarative config object
     * @param {object} [routeOpts] - Per-route options (validated and merged for all pipeline types)
     */
    get(path, pipeline, routeOpts) {
      addRoute(router, 'GET', path, pipeline, routeOpts);
      return router;
    },
     ...
     * @param {object} [routeOpts] - Per-route options (validated and merged for all pipeline types)
     */
    post(path, pipeline, routeOpts) {
      addRoute(router, 'POST', path, pipeline, routeOpts);
      return router;
    },
     ...
     * @param {object} [routeOpts] - Per-route options (validated and merged for all pipeline types)
     */
    put(path, pipeline, routeOpts) {
      addRoute(router, 'PUT', path, pipeline, routeOpts);
      return router;
    },
     ...
     * @param {object} [routeOpts] - Per-route options (validated and merged for all pipeline types)
     */
    patch(path, pipeline, routeOpts) {
      addRoute(router, 'PATCH', path, pipeline, routeOpts);
      return router;
    },
     ...
     * @param {object} [routeOpts] - Per-route options (validated and merged for all pipeline types)
     */
    delete(path, pipeline, routeOpts) {
      addRoute(router, 'DELETE', path, pipeline, routeOpts);
      return router;
    },

    /**
     * Return a formatted route table string summarizing registered routes,
     * enabled middleware, and transport configuration.
     *
     * @returns {string} - Multi-line route table suitable for logging at startup
     */
    routeTable() {
      return formatRouteTable(router);
    }
  };
...
function addRoute(router, method, path, pipeline, routeOpts = {}) {
  ...
  const optsContext = `route options for ${method} ${path}`;
  validateKeys(opts, ROUTE_OPTION_KEYS, optsContext, strict);
  validateRouteOpts(opts, optsContext);

  if (isConfigObject(pipeline)) {
    validateRouteConfig(pipeline, method, path, strict);
    validateSemanticConfig(method, pipeline, _options.defaults ?? {}, path);

    resolvedPipeline = buildPipeline(method, pipeline, _options.defaults ?? {});
    effectiveOpts = {...opts, ...extractRouteOpts(pipeline)};
    const preferValue =
    ...
    const paginateValue =
      pipeline.paginate !== undefined ? pipeline.paginate : (_options.defaults ?? {}).paginate;
    if (paginateValue !== false && paginateValue !== undefined) {
      effectiveOpts.send = {...(effectiveOpts.send ?? {}), paginate: true};
    }
  }

  if (effectiveOpts.redactHeaders instanceof Set) {
    effectiveOpts.redactHeaders = new Set(effectiveOpts.redactHeaders);
  }
  ...
  router._routes.push({
    method,
    path,
    config: isConfigObject(pipeline) ? pipeline : undefined,
...
function extractRouteOpts(config) {
  ...
  if (config.redactHeaders !== undefined) {
    opts.redactHeaders = config.redactHeaders;
  }
  return opts;
}
...
function mountSubRouter(parent, prefix, child) {
  ...
  for (const entry of child._routes) {
    parent._routes.push({
      method: entry.method,
      path: normalizePath(prefix + entry.path),
      config: entry.config,
      defaults: entry.defaults ?? child._options?.defaults
    });
  }
}
...
function dispatch(router, req, res) {
  const {_dispatcher, _pathIndex, _registry, _transport, _redactSet, _options} = router;
  const {method, url} = req;
  const strictPatch = _options.strictPatch !== false;
  const strictBody = _options.strictBody !== false;
  const {onResponse} = _options;
  const startTime = onResponse ? performance.now() : undefined;
  const redactSet = onResponse ? _redactSet : undefined;
  ...
  if (_transport) {
    ...
    if (stop) {
      invokeTransportHook(onResponse, req, res, startTime, redactSet);
      return;
    }
  }
  ...
  if (match) {
    ...
    if (strictBody && BODY_METHODS_REQUIRE_CT.has(method) && !req.headers['content-type']) {
      rejectWithProblem(res, req, 415, onResponse, startTime, redactSet);
      return;
    }

    // --- REST: PATCH Content-Type enforcement (only for matched routes) ---
  ...
  if (allowed) {
    const methods = [...allowed, 'HEAD', 'OPTIONS'];
    res.setHeader('Allow', methods.join(', '));
    rejectWithProblem(res, req, 405, onResponse, startTime, redactSet);
    return;
  }

  rejectWithProblem(res, req, 404, onResponse, startTime, redactSet);
}

/**
 * End the response with an RFC 9457 problem body and invoke the transport-level
 * observation hook. Consolidates the repeated `endWithProblem + invokeTransportHook`
 * pattern used at dispatch reject sites (415, 405, 404).
 *
 * Headers already set on `res` (e.g. `Allow`, `Accept-Patch`) are preserved by
 * `endWithProblem`.
 *
 * @param {import('node:http').ServerResponse} res - HTTP response object
 * @param {import('node:http').IncomingMessage} req - HTTP request
 * @param {number} statusCode - HTTP status code for the rejection
 * @param {function|undefined} onResponse - Router-level observation hook
 * @param {number|undefined} startTime - High-resolution timestamp from dispatch entry
 * @param {Set<string>|undefined} redactSet - Header names to redact in the response info snapshot
 * @returns {void}
 */
function rejectWithProblem(res, req, statusCode, onResponse, startTime, redactSet) {
  endWithProblem(res, statusCode);
  invokeTransportHook(onResponse, req, res, startTime, redactSet);
}

/**
 * Invoke the router-level `onResponse` hook for transport-level short-circuit
 * responses (404, 405, 415, 429, OPTIONS, CORS preflight).
 *
 * Builds a `responseInfo` snapshot with `source: 'transport'` and passes
 * `undefined` as `domainAcc` (no pipeline ran). Errors are swallowed — observation
 * must not affect the response. Async hook rejections are caught to prevent
 * unhandled rejection warnings.
 *
 * @param {function|undefined} onResponse - Router-level observation hook
 * @param {import('node:http').IncomingMessage} req - HTTP request
 * @param {import('node:http').ServerResponse} res - HTTP response (already sent)
 * @param {number|undefined} startTime - High-resolution timestamp from dispatch entry
 * @param {Set<string>|undefined} redactSet - Header names to redact in the response info snapshot
 */
function invokeTransportHook(onResponse, req, res, startTime, redactSet) {
  ...
  try {
    ...
    const result = onResponse(req, res, responseInfo, undefined);
