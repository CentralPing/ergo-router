...
function dispatch(router, req, res) {
  ...
  if (_transport) {
    ...
    if (stop) {
      invokeTransportHook(onResponse, req, res, startTime);
      return;
    }
  }
  ...
  if (match) {
    ...
    if (strictBody && BODY_METHODS_REQUIRE_CT.has(method) && !req.headers['content-type']) {
      rejectWithProblem(res, req, 415, onResponse, startTime);
      return;
    }
  ...
  if (allowed) {
    const methods = [...allowed, 'HEAD', 'OPTIONS'];
    res.setHeader('Allow', methods.join(', '));
    rejectWithProblem(res, req, 405, onResponse, startTime);
    return;
  }

  rejectWithProblem(res, req, 404, onResponse, startTime);
}
 ...
 * @returns {void}
 */
function rejectWithProblem(res, req, statusCode, onResponse, startTime) {
 ...
 * @param {number|undefined} startTime - High-resolution timestamp from dispatch entry
 */
function invokeTransportHook(onResponse, req, res, startTime) {
  if (!onResponse) return;
  try {
    const responseInfo = {...buildResponseInfo(req, res, startTime), source: 'transport'};
