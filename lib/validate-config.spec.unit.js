...
describe('[Boundary] validate-config', () => {
  ...
  describe('validateRouteConfig()', () => {
    ...
    describe('route option value type validation', () => {
      ...
      it('rejects onResponse: object', () => {
        assert.throws(
          () => validateRouteConfig({execute: noop, onResponse: {}}, 'GET', '/test', true),
          {
            message:
              /Invalid "onResponse" in route config for GET \/test.*expected a function.*got object/
          }
        );
      });

      it('rejects onResponse: string', () => {
        assert.throws(
          () => validateRouteConfig({execute: noop, onResponse: 'handler'}, 'GET', '/test', true),
          {message: /Invalid "onResponse".*expected a function.*got string/}
        );
      });

      it('accepts onResponse: function', () => {
        assert.doesNotThrow(() => {
          validateRouteConfig({execute: noop, onResponse: () => {}}, 'GET', '/test', true);
        });
      });

      it('rejects redactHeaders: object (not a Set)', () => {
        assert.throws(
          () =>
            validateRouteConfig(
              {execute: noop, redactHeaders: {authorization: true}},
              'GET',
              '/test',
              true
            ),
          {
            message:
              /Invalid "redactHeaders" in route config for GET \/test.*expected a Set.*got object/
          }
        );
      });

      it('rejects redactHeaders: array', () => {
        assert.throws(
          () =>
            validateRouteConfig(
              {execute: noop, redactHeaders: ['authorization']},
              'GET',
              '/test',
              true
            ),
          {message: /Invalid "redactHeaders".*expected a Set.*got array/}
        );
      });

      it('rejects redactHeaders: string', () => {
        assert.throws(
          () =>
            validateRouteConfig(
              {execute: noop, redactHeaders: 'authorization'},
          ...
          {message: /Invalid "redactHeaders".*expected a Set.*got string/}
        );
      });

      it('accepts redactHeaders: Set', () => {
        assert.doesNotThrow(() => {
          validateRouteConfig(
            {execute: noop, redactHeaders: new Set(['authorization'])},
            'GET',
            '/test',
            true
          );
      ...
      it('accepts redactHeaders: empty Set', () => {
        assert.doesNotThrow(() => {
          validateRouteConfig({execute: noop, redactHeaders: new Set()}, 'GET', '/test', true);
        });
      });
    });
  ...
  describe('validateRouteOpts()', () => {
    it('rejects send: string', () => {
      assert.throws(() => validateRouteOpts({send: 'fast'}, 'route options for GET /test'), {
        message: /Invalid "send" in route options for GET \/test.*expected object.*got string/
      });
    });

    it('rejects send: null', () => {
      assert.throws(() => validateRouteOpts({send: null}, 'route options for GET /test'), {
        message: /Invalid "send".*expected object.*got null/
      });
    });

    it('rejects send: array', () => {
      assert.throws(() => validateRouteOpts({send: []}, 'route options for GET /test'), {
        message: /Invalid "send".*expected object.*got array/
      });
    });
    ...
    it('accepts send: object', () => {
      assert.doesNotThrow(() => {
        validateRouteOpts({send: {prefer: true}}, 'route options for GET /test');
      });
    });

    it('rejects noSend: string', () => {
      assert.throws(() => validateRouteOpts({noSend: 'yes'}, 'route options for GET /test'), {
        message: /Invalid "noSend" in route options for GET \/test.*expected boolean.*got string/
      });
    });

    it('rejects noSend: object', () => {
      assert.throws(() => validateRouteOpts({noSend: {}}, 'route options for GET /test'), {
        message: /Invalid "noSend".*expected boolean.*got object/
      });
    });
    ...
    it('accepts noSend: true', () => {
      assert.doesNotThrow(() => {
        validateRouteOpts({noSend: true}, 'route options for GET /test');
      });
    });

    it('accepts noSend: false', () => {
      assert.doesNotThrow(() => {
        validateRouteOpts({noSend: false}, 'route options for GET /test');
      });
    });

    it('rejects catchHandler: object', () => {
      assert.throws(() => validateRouteOpts({catchHandler: {}}, 'route options for GET /test'), {
        message:
          /Invalid "catchHandler" in route options for GET \/test.*expected a function.*got object/
      });
    });

    it('rejects catchHandler: string', () => {
      assert.throws(
        () => validateRouteOpts({catchHandler: 'handler'}, 'route options for GET /test'),
        {message: /Invalid "catchHandler".*expected a function.*got string/}
      );
    });
    ...
    it('accepts catchHandler: function', () => {
      assert.doesNotThrow(() => {
        validateRouteOpts({catchHandler: () => {}}, 'route options for GET /test');
      });
    });

    it('rejects onResponse: object', () => {
      assert.throws(() => validateRouteOpts({onResponse: {}}, 'route options for GET /test'), {
        message:
          /Invalid "onResponse" in route options for GET \/test.*expected a function.*got object/
      });
    });

    it('rejects onResponse: string', () => {
      assert.throws(
        () => validateRouteOpts({onResponse: 'handler'}, 'route options for GET /test'),
        {message: /Invalid "onResponse".*expected a function.*got string/}
      );
    });
    ...
    it('accepts onResponse: function', () => {
      assert.doesNotThrow(() => {
        validateRouteOpts({onResponse: () => {}}, 'route options for GET /test');
      });
    });

    it('rejects redactHeaders: array', () => {
      assert.throws(
        () => validateRouteOpts({redactHeaders: ['authorization']}, 'route options for GET /test'),
        {
          message:
            /Invalid "redactHeaders" in route options for GET \/test.*expected a Set.*got array/
        }
      );
    });
    ...
    it('accepts redactHeaders: Set', () => {
      assert.doesNotThrow(() => {
        validateRouteOpts(
          {redactHeaders: new Set(['authorization'])},
          'route options for GET /test'
        );
      });
    });
    ...
    it('passes for empty object', () => {
      assert.doesNotThrow(() => {
        validateRouteOpts({}, 'route options for GET /test');
      });
    });

    it('treats explicit undefined values as not provided', () => {
      assert.doesNotThrow(() => {
        validateRouteOpts(
          {
            send: undefined,
    ...
    it('error messages include the provided context', () => {
      assert.throws(
        () => validateRouteOpts({noSend: 'bad'}, 'route options for DELETE /users/:id'),
        err => {
          assert.match(err.message, /Invalid "noSend"/);
          assert.match(err.message, /route options for DELETE \/users\/:id/);
  ...
  describe('validateRouterOptions()', () => {
    ...
    describe('value type validation', () => {
      ...
      it('rejects onResponse: object', () => {
        assert.throws(() => validateRouterOptions({onResponse: {}}, true), {
          message: /Invalid "onResponse" in router options.*expected a function.*got object/
        });
      });

      it('rejects onResponse: string', () => {
        assert.throws(() => validateRouterOptions({onResponse: 'handler'}, true), {
          message: /Invalid "onResponse" in router options.*expected a function.*got string/
        });
      });

      it('accepts onResponse: function', () => {
        assert.doesNotThrow(() => {
          validateRouterOptions({onResponse: () => {}}, true);
        });
      });

      it('rejects redactHeaders: object (not a Set)', () => {
        assert.throws(() => validateRouterOptions({redactHeaders: {}}, true), {
          message: /Invalid "redactHeaders" in router options.*expected a Set.*got object/
        });
      });

      it('rejects redactHeaders: array', () => {
        assert.throws(() => validateRouterOptions({redactHeaders: ['authorization']}, true), {
          message: /Invalid "redactHeaders".*expected a Set.*got array/
        });
      });
      ...
      it('accepts redactHeaders: Set', () => {
        assert.doesNotThrow(() => {
          validateRouterOptions({redactHeaders: new Set(['authorization'])}, true);
        });
      });

      it('accepts redactHeaders: empty Set', () => {
        assert.doesNotThrow(() => {
          validateRouterOptions({redactHeaders: new Set()}, true);
        });
      });
  ...
  describe('validateSemanticConfig()', () => {
    it('throws when route has body: false + validate: {body: schema}', () => {
      assert.throws(
        () =>
          validateSemanticConfig(
            'POST',
            {body: false, validate: {body: {type: 'object'}}},
            {},
            '/items'
          ),
        {
          message: /Route config for POST \/items has body: false but validate\.body is configured/
        }
      );
    });

    it('throws when defaults has body: false + route has validate: {body: schema}', () => {
      assert.throws(
        () =>
          validateSemanticConfig(
            'PUT',
        ...
        {message: /Route config for PUT \/items\/:id has body: false/}
      );
    });

    it('throws when route has body: false + defaults has validate: {body: schema}', () => {
      assert.throws(
        () =>
          validateSemanticConfig(
            'PATCH',
        ...
        {message: /Route config for PATCH \/items\/:id has body: false/}
      );
    });

    it('throws when both body: false and validate: {body: schema} are in defaults', () => {
      assert.throws(
        () =>
          validateSemanticConfig(
            'POST',
        ...
        {message: /Route config for POST \/data has body: false/}
      );
    });

    it('does NOT throw when body: true + validate: {body: schema}', () => {
      assert.doesNotThrow(() => {
        validateSemanticConfig(
          'POST',
          {body: true, validate: {body: {type: 'object'}}},
          {},
          '/items'
        );
      });
    ...
    it('does NOT throw when body: {} (options object) + validate: {body: schema}', () => {
      assert.doesNotThrow(() => {
        validateSemanticConfig(
          'POST',
          {body: {limit: '1mb'}, validate: {body: {type: 'object'}}},
          {},
          '/items'
        );
      });
    ...
    it('does NOT throw when body: false + validate: {} (no body schema)', () => {
      assert.doesNotThrow(() => {
        validateSemanticConfig('POST', {body: false, validate: {}}, {}, '/items');
      });
    });

    it('does NOT throw when body: false + validate: {query: schema} (only query)', () => {
      assert.doesNotThrow(() => {
        validateSemanticConfig(
          'POST',
          {body: false, validate: {query: {type: 'object'}}},
          {},
          '/items'
        );
      });
    ...
    it('does NOT throw when body: false + validate: true', () => {
      assert.doesNotThrow(() => {
        validateSemanticConfig('POST', {body: false, validate: true}, {}, '/items');
      });
    });

    it('does NOT throw when body: false + validate: false', () => {
      assert.doesNotThrow(() => {
        validateSemanticConfig('POST', {body: false, validate: false}, {}, '/items');
      });
    });

    it('does NOT throw when neither body nor validate is configured', () => {
      assert.doesNotThrow(() => {
        validateSemanticConfig('POST', {}, {}, '/items');
      });
    });

    it('error message includes method and path', () => {
      assert.throws(
        () =>
          validateSemanticConfig(
            'POST',
        ...
        err => {
          ...
          assert.match(err.message, /\/users\/:id\/posts/);
          return true;
        }
      );
    });
  ...
  describe('key set consistency', () => {
    ...
    it('ROUTE_OPTION_KEYS matches the exact set of extractRouteOpts keys', () => {
      const expected = new Set(['send', 'noSend', 'catchHandler', 'onResponse', 'redactHeaders']);
