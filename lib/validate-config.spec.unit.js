...
describe('[Boundary] validate-config', () => {
  ...
  describe('key set consistency', () => {
    ...
    it('PIPELINE_KEYS contains exactly 18 keys', () => {
      assert.equal(PIPELINE_KEYS.size, 18);
    });

    it('ROUTE_OPTION_KEYS contains exactly 3 keys', () => {
      assert.equal(ROUTE_OPTION_KEYS.size, 3);
    });

    it('VALID_ROUTER_OPTIONS_KEYS contains exactly 7 keys', () => {
      assert.equal(VALID_ROUTER_OPTIONS_KEYS.size, 7);
    });
  });
});
