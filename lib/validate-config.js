...
export function validateRouteConfig(config, method, path, strict) {
  ...
  if (Object.hasOwn(config, 'use') && config.use !== false && !Array.isArray(config.use)) {
    throw new Error(
      `Invalid "use" in ${context}: expected an array or false, got ${typeof config.use}.`
    );
  }

  if (
    Object.hasOwn(config, 'openapi') &&
    (config.openapi === null || typeof config.openapi !== 'object' || Array.isArray(config.openapi))
  ) {
    throw new Error(
      `Invalid "openapi" in ${context}: expected a plain object, got ${config.openapi === null ? 'null' : typeof config.openapi}.`
    );
  }
}
...
export function validateDefaults(defaults, strict) {
  ...
  if (Object.hasOwn(defaults, 'use') && defaults.use !== false && !Array.isArray(defaults.use)) {
    throw new Error(
      `Invalid "use" in router defaults: expected an array or false, got ${typeof defaults.use}.`
    );
  }
}
