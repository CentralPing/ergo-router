...
export function validateRouteConfig(config, method, path, strict) {
  ...
  if (Object.hasOwn(config, 'onResponse') && typeof config.onResponse !== 'function') {
    throw new Error(
      `Invalid "onResponse" in ${context}: expected a function, got ${displayType(config.onResponse)}.`
    );
  }

  if (Object.hasOwn(config, 'redactHeaders') && !(config.redactHeaders instanceof Set)) {
    throw new Error(
      `Invalid "redactHeaders" in ${context}: expected a Set, got ${displayType(config.redactHeaders)}.`
    );
  }
}
