 ...
 * @type {Set<string>}
 */
export const ROUTE_OPTION_KEYS = new Set([
  'send',
  'noSend',
  'catchHandler',
  'onResponse',
...
export function validateRouteConfig(config, method, path, strict) {
  ...
  validateRouteOpts(config, context);
}
...
/**
 * Validate cross-key semantic consistency of a resolved route configuration.
 *
 * Detects contradictions that are guaranteed to produce runtime errors: specifically,
 * `body: false` combined with `validate: {body: schema}`. Body parsing is disabled but
 * body validation is configured — this always results in a 500 at request time because
 * `acc.body` is never populated.
 *
 * Always throws regardless of `strict` setting (same rationale as value type errors:
 * a semantic contradiction will always cause a runtime error).
 *
 * @param {string} method - HTTP method (e.g., "POST")
 * @param {object} routeConfig - Per-route configuration object
 * @param {object} defaults - Router-level defaults
 * @param {string} path - Route path (e.g., "/users/:id")
 * @returns {void}
 * @throws {Error} When a semantic contradiction is detected
 */
export function validateSemanticConfig(method, routeConfig, defaults, path) {
  const resolvedBody = routeConfig.body !== undefined ? routeConfig.body : defaults.body;

  if (resolvedBody !== false) return;

  const resolvedValidate =
    routeConfig.validate !== undefined ? routeConfig.validate : defaults.validate;

  if (!resolvedValidate || typeof resolvedValidate !== 'object') return;

  if (Object.hasOwn(resolvedValidate, 'body')) {
    throw new Error(
      `Route config for ${method} ${path} has body: false but validate.body is configured. ` +
...
/**
 * Validate route option value types for programmatic `routeOpts` or within
 * a declarative route config object.
 *
 * Checks `send` (object), `noSend` (boolean), `catchHandler` (function),
 * `onResponse` (function), and `redactHeaders` (Set) when present. Does NOT validate keys — callers are
 * responsible for key validation via `validateKeys()` with the appropriate key
 * set (`ROUTE_OPTION_KEYS` for programmatic opts, `VALID_ROUTE_CONFIG_KEYS`
 * for declarative config objects).
 *
 * @param {object} opts - The route options object (or full config object)
 * @param {string} context - Description for error messages (e.g., "route options for GET /users/:id")
 * @returns {void}
 * @throws {Error} When value types are invalid
 */
export function validateRouteOpts(opts, context) {
  if (
    Object.hasOwn(opts, 'send') &&
    (typeof opts.send !== 'object' || opts.send === null || Array.isArray(opts.send))
  ) {
    throw new Error(
      `Invalid "send" in ${context}: expected object, got ${displayType(opts.send)}.`
    );
  }

  if (Object.hasOwn(opts, 'noSend') && typeof opts.noSend !== 'boolean') {
    throw new Error(
      `Invalid "noSend" in ${context}: expected boolean, got ${displayType(opts.noSend)}.`
    );
  }

  if (Object.hasOwn(opts, 'catchHandler') && typeof opts.catchHandler !== 'function') {
    throw new Error(
      `Invalid "catchHandler" in ${context}: expected a function, got ${displayType(opts.catchHandler)}.`
    );
  }

  if (Object.hasOwn(opts, 'onResponse') && typeof opts.onResponse !== 'function') {
    throw new Error(
      `Invalid "onResponse" in ${context}: expected a function, got ${displayType(opts.onResponse)}.`
    );
  }

  if (Object.hasOwn(opts, 'redactHeaders') && !(opts.redactHeaders instanceof Set)) {
    throw new Error(
      `Invalid "redactHeaders" in ${context}: expected a Set, got ${displayType(opts.redactHeaders)}.`
    );
  }
}
...
export function validateRouterOptions(options, strict) {
  ...
  if (Object.hasOwn(options, 'timing') && !isValidMiddlewareOption(options.timing)) {
    throw new Error(
      `Invalid "timing" in router options: expected boolean or object, got ${displayType(options.timing)}.`
    );
  }
  ...
  if (Object.hasOwn(options, 'onResponse') && typeof options.onResponse !== 'function') {
    throw new Error(
      `Invalid "onResponse" in router options: expected a function, got ${displayType(options.onResponse)}.`
    );
  }

  if (Object.hasOwn(options, 'redactHeaders') && !(options.redactHeaders instanceof Set)) {
    throw new Error(
      `Invalid "redactHeaders" in router options: expected a Set, got ${displayType(options.redactHeaders)}.`
    );
  }
