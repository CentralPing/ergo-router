...
describe('[Boundary] route-table', () => {
  describe('formatRouteTable()', () => {
    ...
    it('does not show body for GET with explicit body:true config (#155)', () => {
      const router = createRouter();
      router.get('/items', {body: true, execute: noop});
    ...
    it('does not show body for GET when defaults include body:true (#155)', () => {
      const router = createRouter({defaults: {body: true}});
      router.get('/items', {execute: noop});
    ...
    it('does not show body for DELETE with explicit body:true config (#155)', () => {
      const router = createRouter();
      router.delete('/items/:id', {body: true, execute: noop});
    ...
    it('only shows preconditionRequired for methods where it enforces (PUT/PATCH)', () => {
      const router = createRouter({
        defaults: {preconditionRequired: true}
      });
      router.get('/items', {execute: noop});
      router.put('/items', {execute: noop});
      router.patch('/items', {execute: noop});
      router.post('/items', {execute: noop});
      ...
      const getLine = lines.find(l => l.includes('GET'));
      const putLine = lines.find(l => l.includes('PUT'));
      const patchLine = lines.find(l => l.includes('PATCH'));
      const postLine = lines.find(l => l.includes('POST'));

      assert.doesNotMatch(getLine, /preconditionRequired/);
      assert.doesNotMatch(postLine, /preconditionRequired/);
      assert.match(putLine, /preconditionRequired/);
      assert.match(patchLine, /preconditionRequired/);
    });
  });
});
