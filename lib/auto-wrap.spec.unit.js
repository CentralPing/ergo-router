...
describe('[Boundary] auto-wrap', () => {
  ...
  describe('send option merging', () => {
    it('router-level send flows through when route has no explicit send', async () => {
      const wrap = createAutoWrap({send: {prettify: true}});
      const handler = wrap([() => ({response: {statusCode: 200, body: {a: 1}}})]);
      const res = createMockRes();
      await handler(createMockReq(), res, {});
      assert.ok(res._body.includes('\n'), 'router-level prettify should apply');
    });

    it('route-level send merges with router-level send (paginate + errorFormatter)', async () => {
      const wrap = createAutoWrap({send: {errorFormatter: () => ({extra: true})}});
      const handler = wrap([() => ({response: {statusCode: 422, detail: 'bad input'}})], {
        send: {paginate: true}
      });
      const res = createMockRes();
      await handler(createMockReq(), res, {});
    ...
    it('route-level send keys override router-level keys on conflict', async () => {
      const wrap = createAutoWrap({send: {prettify: true}});
      const handler = wrap([() => ({response: {statusCode: 200, body: {a: 1}}})], {
        send: {prettify: false}
      });
      const res = createMockRes();
      await handler(createMockReq(), res, {});
      assert.ok(!res._body.includes('\n'), 'route-level prettify:false should override');
    });

    it('paginate and prefer coexist with router-level options', async () => {
      const wrap = createAutoWrap({send: {prettify: true}});
      const handler = wrap([() => ({response: {statusCode: 200, body: {a: 1}}})], {
        send: {paginate: true, prefer: true}
      });
      const res = createMockRes();
      await handler(createMockReq(), res, {});
      assert.ok(res._body.includes('\n'), 'router-level prettify should merge through');
    });
  });
  ...
  describe('timing mode', () => {
    ...
    /**
     * Assert a duration string matches `Number.prototype.toFixed(precision)`.
     * Rejects hardcoded `'0'`, non-numeric placeholders, and wrong precision.
     *
     * @param {string} value - Header value from applyResponseTiming
     * @param {number} precision - Expected decimal places
     */
    function assertToFixedDuration(value, precision) {
      assert.equal(typeof value, 'string', 'timing value must be a string');
      const expected = /^\d+\.\d+$/;
      const integerOnly = /^\d+$/;
      if (precision === 0) {
        assert.match(value, integerOnly, `precision 0 must be integer ms, got ${value}`);
      } else {
        assert.match(value, expected, `precision ${precision} must include a fractional part`);
        const [, frac] = value.split('.');
        assert.equal(
          frac.length,
          precision,
          `expected exactly ${precision} decimal places, got ${frac.length} in ${value}`
        );
      }
      const num = Number(value);
      assert.ok(Number.isFinite(num), `timing value must be finite, got ${value}`);
      assert.ok(num >= 0, `timing value must be non-negative, got ${value}`);
    }
    ...
    it('sets x-response-time header when timing is true', async () => {
      ...
      await handler(createMockReq(), res, {});
      assert.equal(res.statusCode, 200);
      assertToFixedDuration(res.getHeader('x-response-time'), 3);
    });
    ...
    it('uses custom header and precision when timing is an object', async () => {
      ...
      assertToFixedDuration(res.getHeader('server-timing'), 0);
    });
    ...
    it('forwards default precision 3 from resolveTimingConfig', async () => {
      const wrap = createAutoWrap({timing: true});
      const handler = wrap([() => ({response: {statusCode: 200, body: {ok: true}}})]);
      const res = createTimingRes();
      await handler(createMockReq(), res, {});
    ...
    it('forwards custom precision from resolveTimingConfig', async () => {
      const wrap = createAutoWrap({timing: {precision: 2}});
      const handler = wrap([() => ({response: {statusCode: 200, body: {ok: true}}})]);
      const res = createTimingRes();
      await handler(createMockReq(), res, {});
      assertToFixedDuration(res.getHeader('x-response-time'), 2);
    });
  });
  ...
  describe('OTEL span lifecycle', () => {
    ...
    it('ends span with status on successful pipeline', async () => {
      ...
      assert.equal(setAttr.key, ATTR_HTTP_RESPONSE_STATUS_CODE);
  ...
  describe('onResponse hook', () => {
    ...
    it('hook receives responseInfo with correct shape and source: pipeline', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
      ...
      assert.equal(hookInfo.source, 'pipeline');
    });
    ...
    it('fires on noSend path with correct responseInfo and source: pipeline', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
        onResponse(req, res, responseInfo) {
          hookInfo = responseInfo;
  ...
  describe('send() error boundary', () => {
    function createThrowingSendRes() {
      const res = createMockRes();
      const origEnd = res.end.bind(res);
    ...
    it('non-catchFn path: emits error to listeners when send throws', async () => {
      const wrap = createAutoWrap();
      const handler = wrap([() => ({response: {statusCode: 200, body: {ok: true}}})]);
      const res = createThrowingSendRes();
      const errors = [];
      res.on('error', e => errors.push(e));
      await handler(createMockReq(), res, {});
      assert.equal(errors.length, 1);
      assert.equal(errors[0].message, 'send failure');
    });

    it('non-catchFn path: ends response with 500 when send throws', async () => {
      const wrap = createAutoWrap();
      const handler = wrap([() => ({response: {statusCode: 200, body: {ok: true}}})]);
      const res = createThrowingSendRes();
      await handler(createMockReq(), res, {});
    ...
    it('non-catchFn path: OTEL span still finalized when send throws', async () => {
      const calls = [];
      const span = {
        setAttribute(key, value) {
          calls.push({method: 'setAttribute', key, value});
      ...
      const recEx = calls.find(c => c.method === 'recordException');
      assert.ok(recEx, 'should recordException for send error');
      assert.equal(recEx.err.message, 'send failure');

      const setAttr = calls.find(c => c.method === 'setAttribute');
      assert.ok(setAttr, 'should setAttribute http.response.status_code');
      assert.equal(setAttr.key, ATTR_HTTP_RESPONSE_STATUS_CODE);
      assert.equal(setAttr.value, 500);

      const end = calls.find(c => c.method === 'end');
      assert.ok(end, 'should end the span');
    });

    it('non-catchFn path: onResponse hooks still fire with status 500 when send throws', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
        onResponse(req, res, responseInfo) {
          hookInfo = responseInfo;
      ...
      const res = createThrowingSendRes();
      await handler(createMockReq(), res, {});
      assert.ok(hookInfo, 'onResponse hook should fire');
      assert.equal(hookInfo.statusCode, 500);
      assert.equal(hookInfo.source, 'pipeline');
    });

    it('catchFn path: emits error, ends response, finalizes span, fires hooks when send throws', async () => {
      const calls = [];
      const span = {
        setAttribute(key, value) {
          calls.push({method: 'setAttribute', key, value});
      ...
      assert.equal(errors.length, 1, 'error emitted to listeners');
      assert.equal(errors[0].message, 'send failure');
      assert.equal(res.statusCode, 500, 'response status set to 500');
      assert.ok(res.writableEnded, 'response ended');

      const recEx = calls.find(c => c.method === 'recordException');
      assert.ok(recEx, 'span recordException called');

      const setAttr = calls.find(c => c.method === 'setAttribute');
      assert.ok(setAttr, 'span setAttribute called');
      assert.equal(setAttr.key, ATTR_HTTP_RESPONSE_STATUS_CODE);
      assert.equal(setAttr.value, 500);

      const end = calls.find(c => c.method === 'end');
      assert.ok(end, 'span ended');

      assert.ok(hookInfo, 'onResponse hook fired');
      assert.equal(hookInfo.statusCode, 500);
    });

    it('does not double-end response when send throws after res.writableEnded', async () => {
      const wrap = createAutoWrap();
      const handler = wrap([() => ({response: {statusCode: 200, body: {ok: true}}})]);
      const res = createMockRes();
      let endCallCount = 0;
      ...
      res.end = function (chunk) {
        endCallCount++;
        const result = origEnd.call(this, chunk);
        if (endCallCount === 1) {
          throw new Error('post-end send failure');
        }
        return result;
      };
      await handler(createMockReq(), res, {});
      assert.equal(
        endCallCount,
        1,
        'end called exactly once (boundary skips because writableEnded)'
      );
      assert.ok(res.writableEnded, 'response is ended');
    });
  });

  describe('onResponse header redaction (#158)', () => {
    it('default: sensitive headers are redacted in responseInfo', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
        onResponse(req, res, responseInfo) {
          hookInfo = responseInfo;
      ...
      await handler(createMockReq(), createMockRes(), {});
      assert.equal(hookInfo.headers['set-cookie'], '[REDACTED]');
      assert.equal(hookInfo.headers['x-custom'], 'visible');
    });

    it('router-level redactHeaders overrides the default set', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
      ...
      await handler(createMockReq(), createMockRes(), {});
      assert.equal(hookInfo.headers['x-secret'], '[REDACTED]');
      assert.equal(hookInfo.headers['set-cookie'], 'session=abc123');
    });

    it('route-level redactHeaders overrides router-level', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
        redactHeaders: new Set(['x-router-secret']),
        onResponse(req, res, responseInfo) {
          hookInfo = responseInfo;
        }
      });
      const handler = wrap(
        [
          (req, res) => {
            res.setHeader('x-router-secret', 'router-hidden');
            res.setHeader('x-route-secret', 'route-hidden');
            return {response: {statusCode: 200, body: 'ok'}};
          }
        ],
        {redactHeaders: new Set(['x-route-secret'])}
      );
      await handler(createMockReq(), createMockRes(), {});
      assert.equal(hookInfo.headers['x-router-secret'], 'router-hidden');
      assert.equal(hookInfo.headers['x-route-secret'], '[REDACTED]');
    });

    it('empty Set disables redaction entirely', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
      ...
      await handler(createMockReq(), createMockRes(), {});
      assert.equal(hookInfo.headers['set-cookie'], 'session=abc123');
      assert.equal(hookInfo.headers['authorization'], 'Bearer token');
    });

    it('redaction applies on catchHandler success path', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
        catchHandler: () => {},
        onResponse(req, res, responseInfo) {
    ...
    it('redaction applies on non-catchHandler path', async () => {
      let hookInfo;
      const wrap = createAutoWrap({
        onResponse(req, res, responseInfo) {
          hookInfo = responseInfo;
      ...
      await handler(createMockReq(), createMockRes(), {});
      assert.equal(hookInfo.headers['authorization'], '[REDACTED]');
    });
  });
});
