...
describe('[Contract] auto-wrap – error emitted to res listeners', () => {
  let baseUrl, close;
  const emittedErrors = [];
  ...
  before(async () => {
    ...
    router.get('/fail', [
      () => {
        const err = new Error('listener test');
        err.statusCode = 422;
        throw err;
      }
    ]);
    ...
    ({baseUrl, close} = await setupServer((req, res) => {
      res.on('error', err => emittedErrors.push(err));
      handle(req, res);
    }));
  });
  ...
  it('emits the caught error to res error listeners', async () => {
    const res = await fetch(`${baseUrl}/fail`);
    assert.equal(res.status, 422);
    assert.equal(emittedErrors.length, 1);
    assert.equal(emittedErrors[0].message, 'listener test');
    assert.equal(emittedErrors[0].statusCode, 422);
  });
});
