...
// --- Deep path: @centralping/ergo/lib/redact-headers ---
// Used by lib/auto-wrap.js, lib/router.js
import {DEFAULT_REDACTED_HEADERS, redactHeaders} from '@centralping/ergo/lib/redact-headers';
...
describe('peer surface: @centralping/ergo main entry', () => {
  it('exports pipeline middleware factories', () => {
    ...
    assert.equal(typeof tracing, 'function');
...
describe('peer surface: @centralping/ergo/lib/tracing', () => {
  it('exports statusFromHttp', () => {
    assert.equal(typeof statusFromHttp, 'function');
  });
});

describe('peer surface: @centralping/ergo/lib/otel-attributes', () => {
  it('exports ATTR_HTTP_RESPONSE_STATUS_CODE as the stable HTTP status attribute', () => {
    assert.equal(typeof ATTR_HTTP_RESPONSE_STATUS_CODE, 'string');
    assert.equal(ATTR_HTTP_RESPONSE_STATUS_CODE, 'http.response.status_code');
  });
});

describe('peer surface: @centralping/ergo/lib/response-info', () => {
  it('exports default buildResponseInfo', () => {
    assert.equal(typeof buildResponseInfo, 'function');
  });
});

describe('peer surface: @centralping/ergo/lib/response-time', () => {
  it('exports default applyResponseTiming', () => {
    assert.equal(typeof applyResponseTiming, 'function');
  });

  it('exports DEFAULT_TIMING_HEADER as a string', () => {
    assert.equal(typeof DEFAULT_TIMING_HEADER, 'string');
  });

  it('exports DEFAULT_TIMING_PRECISION as a number', () => {
    assert.equal(typeof DEFAULT_TIMING_PRECISION, 'number');
  });
});

describe('peer surface: @centralping/ergo/lib/redact-headers', () => {
  it('exports DEFAULT_REDACTED_HEADERS as a Set', () => {
    assert.ok(DEFAULT_REDACTED_HEADERS instanceof Set);
  });

  it('exports redactHeaders as a function', () => {
...
describe('peer surface: @centralping/ergo/lib/security-headers', () => {
  ...
  it('exports DEFAULT_HSTS_MAX_AGE_SECONDS as one year in seconds', () => {
    assert.equal(typeof DEFAULT_HSTS_MAX_AGE_SECONDS, 'number');
