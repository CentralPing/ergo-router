...
describe('[Contract] presets.jsonApi – override semantics', () => {
  ...
  before(async () => {
    const router = createRouter({
      ...presets.jsonApi,
      defaults: {...presets.jsonApi.defaults, logger: true}
    });
  ...
  it('preserves timeout from preset when defaults are extended', async () => {
    const res = await fetch(`${baseUrl}/check`, {
      headers: {accept: 'application/json'}
    });
    assert.equal(res.status, 200);
...
describe('[Contract] presets.jsonApi – timeout active', () => {
  let baseUrl, close;
  ...
  before(async () => {
    ...
    router.get('/with-timeout-override', {
      timeout: false,
      execute: () => ({response: {body: {ok: true}}})
    });
  ...
  it('succeeds for fast handler within default timeout', async () => {
  ...
  it('allows per-route timeout override (disable)', async () => {
    const res = await fetch(`${baseUrl}/with-timeout-override`, {
      headers: {accept: 'application/json'}
    });
    assert.equal(res.status, 200);
    const body = await res.json();
