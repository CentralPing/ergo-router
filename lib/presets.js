 ...
 *   defaults: {...presets.jsonApi.defaults, authorization: {strategies: [{type: 'Bearer', authorizer: verify}]}},
 * });
 */
...
export const presets = Object.freeze({
   ...
   * restricts route-level content negotiation to `application/json`, and
   * applies a default request timeout (ergo's built-in 30 s).
   *
   ...
   * headers, restricts content negotiation to `application/json`, requires
   * the `Idempotency-Key` header for safe at-least-once delivery, and applies
   * a default request timeout (ergo's built-in 30 s).
   *
   * Excludes deployment-specific concerns: auth, CORS origin, rate limiting.
   *
   * @since 0.4.0
   ...
   * negotiation to `application/json`, sets `Cache-Control: public, max-age=300`,
   * and applies a default request timeout (ergo's built-in 30 s).
   *
