...
describe('[Boundary] pipeline-builder', () => {
  ...
  describe('stage ordering', () => {
    ...
    it('places tracing before logger when both configured', () => {
      ...
      const traceIdx = stepNames.indexOf('trace');
      const logIdx = stepNames.indexOf('log');
      const acceptsIdx = stepNames.indexOf('accepts');
  ...
  describe('idempotency pipeline key', () => {
    ...
    it('places idempotency between body and validation', () => {
      ...
      const validationIdx = names.indexOf('validation');
  ...
  describe('use pipeline key', () => {
    it('includes use tuples after Stage 3 (validation) and before Stage 4 (execution)', () => {
      const customMw = () => ({enriched: true});
      const pipeline = buildPipeline(
        'POST',
        {validate: {body: {}}, use: [[customMw, 'enrichment']], execute: noop},
        {}
      );
      const names = tupleNames(pipeline);
      const validationIdx = names.indexOf('validation');
      const enrichmentIdx = names.indexOf('enrichment');
      assert.ok(enrichmentIdx > validationIdx, 'use entries should be after validation');
      const execIdx = pipeline.indexOf(noop);
      const useEntry = pipeline.find(s => Array.isArray(s) && s[1] === 'enrichment');
      const useIdx = pipeline.indexOf(useEntry);
      assert.ok(useIdx < execIdx, 'use entries should be before execute');
    });
    ...
    it('concatenates defaults.use and route.use (defaults first)', () => {
      const defaultMw = [() => ({d: 1}), 'default'];
      const routeMw = [() => ({r: 1}), 'route'];
