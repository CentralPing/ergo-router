...
import createSecurityHeaders, {mergeTransportDefaults} from './security-headers.js';
...
describe('[Boundary] transport/security-headers', () => {
  ...
  describe('HSTS (RFC 6797 §7.2)', () => {
    ...
    it('does NOT set HSTS when trustProxy=true and first X-Forwarded-Proto array entry is http', () => {
      const apply = createSecurityHeaders({trustProxy: true});
      const req = createMockReq({
        socket: {encrypted: false},
    ...
    it('does NOT set HSTS when trustProxy=true and X-Forwarded-Proto is a String object', () => {
      // String objects coerce to 'https' via .split/.toLowerCase; typeof !== 'string' must reject.
      const apply = createSecurityHeaders({trustProxy: true});
      const req = createMockReq({
        socket: {encrypted: false},
        headers: {'x-forwarded-proto': Object('https')}
      });
      const res = createMockRes();
      apply(req, res);
      assert.equal(res.getHeader('strict-transport-security'), undefined);
    ...
    it('throws when hsts is explicitly null', () => {
      assert.throws(() => createSecurityHeaders({hsts: null}), {
        name: 'TypeError',
        message: /strictTransportSecurity/
      });
    });

    it('throws when csp is explicitly null', () => {
      assert.throws(() => createSecurityHeaders({csp: null}), {
        name: 'TypeError',
        message: /contentSecurityPolicy/
      });
    });

    it('ignores __proto__ config keys without replacing the merged prototype', () => {
      const evil = JSON.parse('{"__proto__":{"polluted":true},"frameOptions":"SAMEORIGIN"}');
      const merged = mergeTransportDefaults(evil);
      assert.equal(Object.getPrototypeOf(merged), Object.prototype);
      assert.equal(merged.polluted, undefined);
      assert.equal(merged.frameOptions, 'SAMEORIGIN');

      const apply = createSecurityHeaders(evil);
      const res = createMockRes();
      apply(createMockReq(), res);
