...
/** Visible ASCII characters per RFC 9110 §5.5 VCHAR (%x21-7E). */
 ...
 * @param {function} [config.generate] - Custom ID generator; must return a VCHAR-only string (default: crypto.randomUUID)
...
export default function createRequestId(config = {}) {
  ...
  return function applyRequestId(req, res) {
    ...
    if (!id) {
      id = generate();
      if (typeof id !== 'string' || id.length > MAX_ID_LENGTH || !VCHAR_RE.test(id)) {
        throw new TypeError(
          `generate() must return a VCHAR-only string (visible ASCII \\x21-\\x7E, max ${MAX_ID_LENGTH} chars)`
        );
      }
    }
