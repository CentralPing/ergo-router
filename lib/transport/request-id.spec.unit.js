...
describe('[Boundary] transport/request-id', () => {
  ...
  describe('generate() output validation', () => {
    it('throws when generate returns CRLF', () => {
      const apply = createRequestId({generate: () => 'bad\r\nvalue'});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns non-string', () => {
      const apply = createRequestId({generate: () => 12345});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns oversized string', () => {
      const apply = createRequestId({generate: () => 'x'.repeat(200)});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns string with null byte', () => {
      const apply = createRequestId({generate: () => 'bad\0value'});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns DEL character (\\x7F)', () => {
      const apply = createRequestId({generate: () => 'id\x7Fvalue'});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns ESC character (\\x1B)', () => {
      const apply = createRequestId({generate: () => 'id\x1Bvalue'});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns space character', () => {
      const apply = createRequestId({generate: () => 'id with spaces'});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns tab character', () => {
      const apply = createRequestId({generate: () => 'id\twith\ttabs'});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns non-ASCII character (\\x80)', () => {
      const apply = createRequestId({generate: () => 'id\x80value'});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('throws when generate returns empty string', () => {
      const apply = createRequestId({generate: () => ''});
      assert.throws(() => apply(createMockReq(), createMockRes()), TypeError);
    });

    it('accepts VCHAR boundary character ! (\\x21)', () => {
      const apply = createRequestId({generate: () => '!valid-id!'});
      const res = createMockRes();
      const id = apply(createMockReq(), res);
      assert.equal(id, '!valid-id!');
    });

    it('accepts VCHAR boundary character ~ (\\x7E)', () => {
      const apply = createRequestId({generate: () => '~valid-id~'});
      const res = createMockRes();
      const id = apply(createMockReq(), res);
      assert.equal(id, '~valid-id~');
  ...
  describe('trustProxy', () => {
    ...
    it('ignores incoming header when trustProxy is a non-boolean truthy value', () => {
      const apply = createRequestId({trustProxy: 'true'});
      const req = createMockReq({headers: {'x-request-id': validUUID}});
      const res = createMockRes();
      const id = apply(req, res);
