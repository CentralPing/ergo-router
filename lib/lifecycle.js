 ...
 * @param {{info: function, warn: function, error: function}} [options.log=console] - Logger with `.info()`, `.warn()`, `.error()` methods
 ...
 * @param {(ctx: {log: {info: function, warn: function, error: function}}) => Promise<void>} [options.onStartup] - Async hook called before `server.listen()`.
 *   Receives `{log}`. Rejection prevents the server from starting.
 * @param {(ctx: {log: {info: function, warn: function, error: function}, signal: string}) => Promise<void>} [options.onShutdown] - Async hook called after `server.close()`.
