...
class MethodRegistry {
  ...
  /**
   * Merge another registry into this one (for sub-router mounting).
   * @param {string} prefix - Path prefix to prepend
   * @param {MethodRegistry} other - Registry to merge from
   */
  merge(prefix, other) {
    for (const [path, methods] of other._paths) {
      const fullPath = prefix + path;
      for (const method of methods) {
        this.add(method, fullPath);
      }
    }
  }
}
