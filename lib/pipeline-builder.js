 ...
 *
 * v2 changes:
 * - Tuple format changed from `[fn, getPaths, setPath]` to `[fn, setPath]`
 * - CSRF is wrapped with a method-dispatching adapter (issue for safe, verify for unsafe)
 * - validate no longer uses getPaths (receives full domain accumulator)
 *
 ...
 * 2. **Authorization** — csrf (method-dispatching adapter) → auth (async rejection)
 ...
 * @returns {function} - Middleware `(req, res, domainAcc, responseAcc) => result`
 */
