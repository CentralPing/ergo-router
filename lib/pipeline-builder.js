 ...
 * @property {Array<function|[function, string]>|false} [use] - Custom middleware tuples or bare functions inserted after validation and before execution. Defaults and route arrays are concatenated; `false` disables all.
...
export default function buildPipeline(method, routeConfig, defaults = {}) {
  ...
  if (tracingOpts !== false && tracingOpts !== undefined) {
    pipeline.push([createTracing(tracingOpts), 'trace']);
  }
  ...
  if (paginateActive) {
    negotiation.push([createPaginate(paginateOpts), 'paginate']);
  }
