 ...
 * @property {object|false} [logger] - Logger options or `false` to disable
 * @property {object|boolean|false} [rateLimit] - Rate limiting options, `true` for defaults, or `false`
 * @property {object|false} [accepts] - Content negotiation options or `false`
 * @property {object|boolean|false} [preconditionRequired] - Precondition enforcement options, `true` for PUT/PATCH defaults, or `false`
 * @property {object|boolean|false} [cookie] - Cookie parsing options, `true` for defaults, or `false`
 * @property {object|boolean|false} [url] - URL parsing options, `true` for defaults, or `false` (auto-included for GET/DELETE)
 * @property {object|boolean|false} [jsonApiQuery] - JSON:API query parsing options, or `false`
 * @property {object|boolean|false} [prefer] - Prefer header parsing options, `true` for defaults, or `false`
 * @property {object|boolean|false} [securityHeaders] - Security headers options, `true` for defaults, or `false`
 * @property {object|boolean|false} [cacheControl] - Cache-Control options, `true` for defaults, or `false`
 * @property {object|false} [csrf] - CSRF options or `false`
 * @property {object|false} [authorization] - Authorization options or `false`
 * @property {object|false} [body] - Body parsing options or `false` (auto-included for POST/PUT/PATCH)
 * @property {object|boolean|false} [idempotency] - Idempotency-Key options, `true` for defaults, or `false`
 * @property {object|false} [validate] - Validation schemas `{body, query, params}` or `false`
 * @property {object|false} [timeout] - Timeout options or `false`
 * @property {object|false} [compress] - Compression options or `false`
