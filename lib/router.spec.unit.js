...
describe('[Module] router dispatch', () => {
  ...
  describe('PATCH Content-Type enforcement (strictPatch=true)', () => {
    ...
    it('returns 404 (not 415) for PATCH to nonexistent path with wrong Content-Type', () => {
      const router = createRouter({strictPatch: true});
      router.patch('/items/:id', (req, res) => res.end());
      const {res} = dispatch(router, {
        method: 'PATCH',
    ...
    it('returns 405 (not 415) for PATCH to GET-only path with wrong Content-Type', () => {
      const router = createRouter({strictPatch: true});
      router.get('/items', (req, res) => res.end());
      const {res} = dispatch(router, {
        method: 'PATCH',
        url: '/items',
        headers: {'content-type': 'text/plain'}
    ...
    it('returns 415 for PATCH to registered PATCH path with wrong Content-Type', () => {
      const router = createRouter({strictPatch: true});
      router.patch('/items/:id', (req, res) => res.end());
      const {res} = dispatch(router, {
        method: 'PATCH',
  ...
  describe('config validation at registration time', () => {
    it('throws for unknown route config keys (strict=true default)', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.throws(() => router.get('/test', {execute: () => ({}), validatte: {}}), {
        message: /Unknown config key "validatte" in route config for GET \/test/
      });
    });

    it('throws for missing execute in config object', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.throws(() => router.post('/items', {validate: {}}), {
        message: /Missing "execute" function in route config for POST \/items/
      });
    });

    it('throws for non-function execute in config object', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.throws(() => router.put('/items/:id', {execute: 'handler'}), {
        message: /Invalid "execute" in route config for PUT \/items\/:id/
      });
    });

    it('does not throw for a valid config object', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.doesNotThrow(() => {
        router.get('/data', {execute: () => ({response: {body: {ok: true}}})});
      });
    });

    it('strict: false warns instead of throwing for unknown route config keys', () => {
      ...
      try {
        ...
        assert.doesNotThrow(() => {
          router.get('/test', {execute: () => ({}), bogus: true});
        });
        assert.equal(warnMock.mock.calls.length, 1);
        assert.match(warnMock.mock.calls[0].arguments[0], /Unknown config key "bogus"/);
      } finally {
        warnMock.mock.restore();
      }
    });

    it('throws for unknown createRouter() options (strict=true default)', () => {
      assert.throws(() => createRouter({trasport: {}}), {
        message: /Unknown config key "trasport" in router options.*did you mean "transport"/
      });
    });

    it('throws for unknown keys in defaults', () => {
      assert.throws(() => createRouter({defaults: {validatte: {}}}), {
        message: /Unknown config key "validatte" in router defaults/
      });
    });

    it('throws for "execute" in defaults (not valid as a default)', () => {
      assert.throws(() => createRouter({defaults: {execute: () => {}}}), {
        message: /Unknown config key "execute" in router defaults/
      });
    });

    it('does not validate function or array pipelines', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.doesNotThrow(() => {
        router.get('/fn', (req, res) => res.end('ok'));
        router.get('/arr', [() => ({response: {body: {ok: true}}})]);
      });
    });

    it('throws for body: false + validate: {body: schema} contradiction', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.throws(
        () =>
          router.post('/test', {
        ...
        {message: /Route config for POST \/test has body: false but validate\.body is configured/}
      );
    });

    it('throws for body: false in defaults + validate: {body: schema} in route', () => {
      const router = createRouter({
        transport: {requestId: false, security: false},
        defaults: {body: false}
      });
      assert.throws(
        ...
        {
          message:
            /Route config for PUT \/items\/:id has body: false but validate\.body is configured/
        }
      );
    });

    it('does not throw for body: true + validate: {body: schema}', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.doesNotThrow(() => {
        router.post('/items', {
          execute: () => ({}),
  ...
  describe('programmatic routeOpts validation at registration time', () => {
    it('throws for invalid routeOpts.noSend (non-boolean)', () => {
      const router = createRouter();
      assert.throws(() => router.get('/test', (req, res) => res.end(), {noSend: 'yes'}), {
        message: /Invalid "noSend" in route options for GET \/test.*expected boolean.*got string/
      });
    });

    it('throws for invalid routeOpts.catchHandler (non-function)', () => {
      const router = createRouter();
      assert.throws(() => router.get('/test', (req, res) => res.end(), {catchHandler: {}}), {
        message:
          /Invalid "catchHandler" in route options for GET \/test.*expected a function.*got object/
      });
    });

    it('throws for invalid routeOpts.onResponse (non-function)', () => {
      const router = createRouter();
      assert.throws(() => router.get('/test', (req, res) => res.end(), {onResponse: 'handler'}), {
        message:
          /Invalid "onResponse" in route options for GET \/test.*expected a function.*got string/
      });
    });

    it('throws for invalid routeOpts.send (non-object)', () => {
      const router = createRouter();
      assert.throws(() => router.get('/test', (req, res) => res.end(), {send: 'fast'}), {
        message: /Invalid "send" in route options for GET \/test.*expected object.*got string/
      });
    });

    it('throws for unknown routeOpts key (with suggestion)', () => {
      const router = createRouter();
      assert.throws(() => router.get('/test', (req, res) => res.end(), {noSned: true}), {
        message:
          /Unknown config key "noSned" in route options for GET \/test.*did you mean "noSend"/
      });
    });

    it('does not throw for valid programmatic routeOpts', () => {
      const router = createRouter();
      assert.doesNotThrow(() => {
        router.get('/test', (req, res) => res.end(), {
          noSend: true,
          catchHandler: () => {},
          onResponse: () => {},
          send: {prefer: true}
    ...
    it('does not throw for empty programmatic routeOpts', () => {
      const router = createRouter();
      assert.doesNotThrow(() => {
        router.get('/test', (req, res) => res.end(), {});
      });
    ...
    it('strict: false warns for unknown programmatic routeOpts key', () => {
      const warnMock = mock.method(console, 'warn', () => {});
      try {
        const router = createRouter({strict: false});
        assert.doesNotThrow(() => {
          router.get('/test', (req, res) => res.end(), {bogus: true});
        });
        assert.equal(warnMock.mock.calls.length, 1);
        assert.match(
          warnMock.mock.calls[0].arguments[0],
          /Unknown config key "bogus" in route options for GET \/test/
        );
      } finally {
        warnMock.mock.restore();
      }
    ...
    it('does not validate routeOpts for declarative config objects', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.doesNotThrow(() => {
    ...
    it('validates routeOpts for array pipelines', () => {
      const router = createRouter();
      assert.throws(() => router.post('/test', [() => ({})], {catchHandler: 'not-a-function'}), {
        message:
          /Invalid "catchHandler" in route options for POST \/test.*expected a function.*got string/
      });
    });

    it('validates routeOpts for declarative config with third argument', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.throws(
        () => router.get('/test', {execute: () => ({})}, {catchHandler: 'not-a-function'}),
        {
          message:
            /Invalid "catchHandler" in route options for GET \/test.*expected a function.*got string/
        }
      );
    });

    it('rejects unknown keys in routeOpts with declarative config', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.throws(() => router.get('/test', {execute: () => ({})}, {bogus: true}), {
        message: /Unknown config key "bogus" in route options for GET \/test/
      });
    });

    it('normalizes null routeOpts without throwing', () => {
      const router = createRouter({transport: {requestId: false, security: false}});
      assert.doesNotThrow(() => {
        router.get('/test', {execute: () => ({})}, null);
      });
    });

    it('throws for invalid programmatic redactHeaders (non-Set)', () => {
      const router = createRouter();
      assert.throws(
        () => router.get('/test', (req, res) => res.end(), {redactHeaders: ['authorization']}),
        {
          message:
    ...
    it('throws TypeError when routeOpts is a non-plain-object (function)', () => {
      const router = createRouter();
      assert.throws(
        () =>
          router.get(
        ...
        {
          name: 'TypeError',
          message: /Expected route options for GET \/test to be a plain object/
        }
      );
    });

    it('clones redactHeaders Set to prevent post-registration mutation', () => {
      const router = createRouter();
      const headers = new Set(['authorization']);
      router.get('/test', (req, res) => res.end(), {redactHeaders: headers});
      headers.clear();
      const stored = router._dispatcher.find('GET', '/test').store.options;
      assert.equal(stored.redactHeaders.size, 1);
      assert.ok(stored.redactHeaders.has('authorization'));
    });
  });
});
