...
describe('[Boundary] error-response', () => {
  ...
  it('includes instance from x-request-id header', () => {
    const res = createMockRes();
    res.setHeader('x-request-id', 'req-id-42');
    endWithProblem(res, 404);
    const body = JSON.parse(res._body);
    assert.equal(body.instance, 'urn:uuid:req-id-42');
  });

  it('omits instance when no x-request-id header exists', () => {
    const res = createMockRes();
    endWithProblem(res, 404);
    const body = JSON.parse(res._body);
  ...
  it('does not overwrite instance provided in opts', () => {
    const res = createMockRes();
    res.setHeader('x-request-id', 'header-id');
    endWithProblem(res, 404, {instance: 'urn:uuid:explicit-id'});
    const body = JSON.parse(res._body);
    assert.equal(body.instance, 'urn:uuid:explicit-id');
  });
