...
describe('[Contract] router – declarative route config', () => {
  ...
  before(async () => {
    ({baseUrl, close} = await makeServer(
      r => {
        r.get('/data', {
          execute: () => ({statusCode: 200, body: {source: 'declarative'}})
        });
        ...
        r.get('/array', [() => ({statusCode: 200, body: {source: 'array'}})]);
      },
      {
...
describe('[Contract] router – preconditionRequired', () => {
  ...
  before(async () => {
    ({baseUrl, close} = await makeServer(
      r => {
        r.put('/items/:id', {
          execute: () => ({statusCode: 200, body: {updated: true}})
        });

        r.patch('/items/:id', {
          execute: () => ({statusCode: 200, body: {patched: true}})
        });

        r.get('/items/:id', {
          execute: (req, res, acc) => ({statusCode: 200, body: {id: acc.route.params.id}})
        });
      },
      {
        transport: {requestId: false, security: false},
...
describe('[Contract] router – pipeline rateLimit', () => {
  ...
  before(async () => {
    ({baseUrl, close} = await makeServer(
      r => {
        r.get('/limited', {
          execute: () => ({statusCode: 200, body: {ok: true}})
        });
      },
      {
        transport: {requestId: false, security: false},
    ...
    ({baseUrl, close} = await makeServer(
      r => {
        r.put('/items/:id', {
          ...
          execute: (req, res, acc) => ({
            statusCode: 200,
            body: {id: acc.route.params.id, name: 'updated'}
          })
        });
