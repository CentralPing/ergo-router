...
describe('[Contract] router – PATCH strictPatch', () => {
  ...
  it('returns 404 (not 415) for PATCH to nonexistent path with wrong Content-Type', async () => {
  ...
  it('returns 405 (not 415) for PATCH to GET-only path with wrong Content-Type', async () => {
    const {baseUrl: url2, close: close2} = await makeServer(
      r => {
        r.get('/items', (req, res) => res.end('ok'));
      },
      {strictPatch: true}
    );
    const res = await fetch(`${url2}/items`, {
      method: 'PATCH',
      headers: {'content-type': 'text/plain'},
      body: 'data'
    });
...
// ---------------------------------------------------------------------------
// onResponse header redaction (#158)
// ---------------------------------------------------------------------------
describe('[Contract] router – onResponse header redaction (#158)', () => {
  it('transport-level onResponse redacts sensitive headers by default', async () => {
    let hookInfo;
    const {baseUrl, close} = await makeServer(
      r => {
        r.get('/exists', {execute: () => ({response: {body: {ok: true}}})});
    ...
    try {
      const res = await fetch(`${baseUrl}/nope`);
      assert.equal(res.status, 404);
      assert.ok(hookInfo, 'onResponse should fire');
      assert.equal(hookInfo.source, 'transport');
    } finally {
      await close();
    }
  ...
  it('pipeline-level onResponse redacts set-cookie by default', async () => {
    let hookInfo;
    const {baseUrl, close} = await makeServer(
      r => {
        r.get('/cookie-route', {
    ...
    try {
      const res = await fetch(`${baseUrl}/cookie-route`);
      assert.equal(res.status, 200);
      assert.ok(hookInfo, 'onResponse should fire');
  ...
  it('router-level redactHeaders overrides default set on transport path', async () => {
    let hookInfo;
    const {baseUrl, close} = await makeServer(
      r => {
        r.get('/exists', {execute: () => ({response: {body: {ok: true}}})});
      ...
      {
        transport: {requestId: false, security: {}},
        redactHeaders: new Set(['x-frame-options']),
        onResponse(req, res, responseInfo) {
          hookInfo = responseInfo;
        }
      }
    ...
    try {
      ...
      assert.equal(hookInfo.source, 'transport');
      assert.equal(
        hookInfo.headers['x-frame-options'],
        '[REDACTED]',
        'custom redact set should redact x-frame-options on transport path'
      );
      assert.notEqual(
        hookInfo.headers['x-content-type-options'],
        '[REDACTED]',
        'headers outside the custom redact set should remain visible'
      );
    } finally {
      await close();
    }
  ...
  it('empty redactHeaders disables redaction on pipeline path', async () => {
    let hookInfo;
    const {baseUrl, close} = await makeServer(
      r => {
        r.get('/cookie-route', {
    ...
    try {
      ...
      assert.equal(res.status, 200);
      assert.ok(hookInfo, 'onResponse should fire');
      assert.equal(hookInfo.headers['set-cookie'], 'session=visible');
    } finally {
      await close();
    }
  });

  it('route-level redactHeaders in declarative config overrides router default', async () => {
    let hookInfo;
    const {baseUrl, close} = await makeServer(
      r => {
        r.get('/custom-redact', {
          execute: (req, res) => {
            res.setHeader('set-cookie', 'session=secret');
            res.setHeader('x-custom-secret', 'sensitive-value');
            return {response: {body: {ok: true}}};
          },
          redactHeaders: new Set(['x-custom-secret']),
          onResponse(req, res, responseInfo) {
            hookInfo = responseInfo;
          }
        });
    ...
    try {
      const res = await fetch(`${baseUrl}/custom-redact`);
      assert.equal(res.status, 200);
      assert.ok(hookInfo, 'onResponse should fire');
      assert.equal(hookInfo.headers['x-custom-secret'], '[REDACTED]');
      assert.equal(hookInfo.headers['set-cookie'], 'session=secret');
    } finally {
      await close();
    }
  });
