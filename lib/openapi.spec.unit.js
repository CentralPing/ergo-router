...
describe('[Boundary] openapi', () => {
  ...
  describe('path parameter conversion', () => {
    ...
    it('converts multi-param segments :lat-:lng', async () => {
      const router = createRouter();
      router.get('/coords/:lat-:lng', {execute: noop});
      const spec = generateOpenAPI(router);

      assert.ok(spec.paths['/coords/{lat}-{lng}']);
      const params = spec.paths['/coords/{lat}-{lng}'].get.parameters;
      assert.equal(params.length, 2);
      assert.equal(params[0].name, 'lat');
      assert.equal(params[1].name, 'lng');
      await assertValidOpenAPI(spec);
    });

    it('strips regex constraints from param names', async () => {
      const router = createRouter();
      router.get('/users/:id(^\\d+)', {execute: noop});
      const spec = generateOpenAPI(router);
  ...
  describe('request body', () => {
    ...
    it('advertises all accepted content types in requestBody', async () => {
      const router = createRouter();
      router.post('/multi', {
        accepts: {types: ['application/json', 'application/xml', 'text/plain']},
        validate: {body: {type: 'object'}},
        execute: noop
      });
      const spec = generateOpenAPI(router);

      const content = spec.paths['/multi'].post.requestBody.content;
      assert.ok(content['application/json']);
      assert.ok(content['application/xml']);
      assert.ok(content['text/plain']);
      assert.deepStrictEqual(content['application/json'].schema, {type: 'object'});
      assert.deepStrictEqual(content['application/xml'].schema, {type: 'object'});
      await assertValidOpenAPI(spec);
    });
  ...
  describe('openapi annotations', () => {
    ...
    it('produces default 200 response for GET when no annotation responses', async () => {
      const router = createRouter();
    ...
    it('produces default 201 response for POST when no annotation responses', async () => {
      const router = createRouter();
      router.post('/users', {execute: noop});
      const spec = generateOpenAPI(router);

      assert.deepStrictEqual(spec.paths['/users'].post.responses, {
        201: {description: 'Resource created'}
      });
      await assertValidOpenAPI(spec);
    });

    it('produces default 204 response for DELETE when no annotation responses', async () => {
      const router = createRouter();
      router.delete('/users/:id', {execute: noop});
      const spec = generateOpenAPI(router);

      assert.deepStrictEqual(spec.paths['/users/{id}'].delete.responses, {
        204: {description: 'No content'}
      });
      await assertValidOpenAPI(spec);
    });

    it('produces default 200 response for PUT when no annotation responses', async () => {
      const router = createRouter();
      router.put('/users/:id', {execute: noop});
      const spec = generateOpenAPI(router);

      assert.deepStrictEqual(spec.paths['/users/{id}'].put.responses, {
        200: {description: 'Successful response'}
      });
      await assertValidOpenAPI(spec);
    });

    it('produces default 200 response for PATCH when no annotation responses', async () => {
      const router = createRouter();
      router.patch('/users/:id', {execute: noop});
      const spec = generateOpenAPI(router);

      assert.deepStrictEqual(spec.paths['/users/{id}'].patch.responses, {
        200: {description: 'Successful response'}
      });
      await assertValidOpenAPI(spec);
    });

    it('annotation responses override method-aware default for POST', async () => {
      const router = createRouter();
      router.post('/items', {
        openapi: {
          responses: {
            200: {description: 'Bulk insert result'},
            400: {description: 'Validation error'}
          }
        },
        execute: noop
      ...
      assert.deepStrictEqual(spec.paths['/items'].post.responses, {
        200: {description: 'Bulk insert result'},
        400: {description: 'Validation error'}
      });
      await assertValidOpenAPI(spec);
    ...
    it('annotation responses override method-aware default for DELETE', async () => {
      const router = createRouter();
      router.delete('/items/:id', {
        openapi: {
          responses: {
            200: {description: 'Deleted item returned'},
      ...
      assert.deepStrictEqual(spec.paths['/items/{id}'].delete.responses, {
        200: {description: 'Deleted item returned'},
        404: {description: 'Item not found'}
      });
      await assertValidOpenAPI(spec);
    ...
    it('passes through arbitrary operation-level fields from annotations', async () => {
      const router = createRouter();
      router.get('/callback', {
        openapi: {
          callbacks: {onEvent: {'{$request.body#/callbackUrl}': {post: {summary: 'Callback'}}}},
      ...
      const op = spec.paths['/callback'].get;
      assert.ok(op.callbacks);
      assert.deepStrictEqual(op.servers, [{url: 'https://override.example.com'}]);
      await assertValidOpenAPI(spec);
    });

    it('annotation overrides derived security', async () => {
      const router = createRouter();
      router.get('/override-auth', {
        authorization: {strategies: [{type: 'Bearer', authorizer: noop}]},
      ...
      const op = spec.paths['/override-auth'].get;
      assert.deepStrictEqual(op.security, [{ApiKey: []}]);
      await assertValidOpenAPI(spec);
    });
  });
  ...
  describe('defaults resolution', () => {
    ...
    it('preserves child-router defaults for mounted routes', async () => {
  ...
  describe('router._routes metadata', () => {
    it('stores config and defaults for declarative routes', () => {
      const defaults = {authorization: {strategies: []}};
      ...
      assert.equal(router._routes[0].defaults, defaults);
    });
    ...
    it('copies child routes with prefixed path and child defaults on mount', () => {
      const childDefaults = {accepts: {types: ['application/json']}};
      const child = createRouter({defaults: childDefaults});
      child.get('/items', {execute: noop});
      ...
      assert.equal(mounted[0].defaults, childDefaults);
      assert.equal(mounted[1].defaults, childDefaults);
    });
  });
});
